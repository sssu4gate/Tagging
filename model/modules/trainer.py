...
class MyEngine(Engine) :
  ...
  def train(engine, mini_batch) :
      ...
      if isinstance(y, torch.LongTensor) or isinstance(y, torch.cuda.LongTensor) :
        ##결과에 대한 구성이 다르기 때문에 accuracy 부분 바꿔줘야 할 듯 
         accuracy = (torch.argmax(y_hat,dim=-1) == y).sum() / float(y.size(0))
      else :
         accuracy = 0

      # model 내의 parameter의 l2 norm의 합-> 보통 학습이 진행됨에 따라 그 값이 커짐 
      p_norm = float(get_parameter_norm(engine.model.parameters()))
      # model 내의 parameter에 대한 gradient의 l2 norm의 합 -> 보통 학습이 진행됨에 따라 값이 작아짐-> g_norm이 커지거나 요동치면 학습이 불안정한 것임
  ...
  def attach(train_engine, validation_engine, verbose=VERBOSE_BATCH_WISE) :
      ...
      #Thus, we can reduce the repeated codes by usin this function.
      def attaching_running_average(engine, metric_name) :
      ...
      for metric_name in training_metric_names :
          attaching_running_average(train_engine, metric_name)
      ...
      for metric_name in validation_metric_names :
          attaching_running_average(validation_engine, metric_name)
  ...
  def check_best(engine) :
      ...
      if loss <= engine.best_loss : # If current epoch return lower validation loss,
