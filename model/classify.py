...
def define_argparser() :
    ...
    p.add_argument('--text_fn',required=True)
    p.add_argument('--label_fn',required=True)
...
def define_field() :
    ...
    With those fields, we can resotre mapping table between words and idice.
    '''
    return (
        data.Field(
            use_vocab=True,
...
def main(config):
    ...
    f = open(config.text_fn,'r')
    lines = f.read().split('\n')
    f.close()
    f = open(config.label_fn,'r')
    labels = f.read().split('\n')
    f.close()
    ...
    with torch.no_grad() :
       ...
       if cnn_best is not None and not config.drop_cnn:
           ...
           model = CNNClassifier(
               ...
               dropout_p=train_config.dropout,
               window_size=train_config.window_sizes,
    ...
    y_hats = y_hats.sum(dim=0) / len(ensemble) #Gett Average
    ...
    for i in range(len(lines)) :
        if classes.itos[indice[i][0]] == labels[i] :
           correct += 1 
        #sys.stdout.write(
         #print('%s' %(
